...
class AppLauncher  {
    ...
    private static void getArgsForEnvelope(Scanner scanner) {
        ...
        try {
            logger.info("Enter the side A of first envelope");
            a = scanner.nextDouble();
            logger.info("Enter the side B of first envelope");
            b = scanner.nextDouble();
            logger.info("Enter the side A of second envelope");
            c = scanner.nextDouble();
            logger.info("Enter the side B of second envelope");
            d = scanner.nextDouble();
        } catch (InputMismatchException e) {
            logger.info("The sides of the model can only be integer or floating point numbers");
        }
    ...
    private static void AnalyzeEnvelope(double a, double b, double c, double d) {
        ...
        if (validator.validate(firstEnvelope) && validator.validate(secondEnvelope)) {
            ...
            if (comparator.compare(firstEnvelope, secondEnvelope)) {
                logger.info("You can put one model in another");
            } else {
                logger.info("You cannot put one model in another");
            }
        }
    }
